...
public:
    int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        int pigs=0;
        int testCases=(minutesToTest/minutesToDie)+1; 
        //Example 1 reference: we are adding 1 because it can be the case when after all 4 test cases pig is alive, then after that last bucket(5th one) is poisonous. (for 1 pig)
        
        while(pow(testCases,pigs)<buckets) pigs++;
